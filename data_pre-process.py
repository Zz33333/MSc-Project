...
def resample_fill(df, output_file_name, freq='1S'):
    ...
    # Resample the data for each second and fill in missing values ​​using quadratic spline interpolation
...
if GPS_PROCESS:
    ...
    # Modify the seconds_elapsed column to 'Time(s)'
...
# Abnormal data processing
    ...
    # 3. Calculate velocity from bike x-axis acceleration
    # Take the velocity that starts with 0 as the initial velocity

    resample_df_bike['speed'] = 0
    sample_time_interval = 0.1

    # Find the time period when it could be stationary based on the x-axis acceleration
# Speed ​​= previous speed + acceleration * time interval
...
if OFFSET_PROCESS:
    ...
    offset = 0
    min_mean_error = float('inf')
    # Set the offset range from -100 to 100s in 0.1s steps.
    for i in range(-1000, 1000):
        # Calculate the mean value of the error, only the time overlap is calculated
        shifted_ax = df_bike['ax(g)'].shift(i)
        #  Remove the part of bike that is nan
...
# 1.Find the stationary time point through the three-axis angular velocity: the time point when the sum of the squares of the three-axis angular velocity is the smallest
...
# 4. Rotate and align IMU data according to the three-axis angle of IMU relative to the navigation coordinate system
